...
// @version      7.3.2
// @description  多背景随机切换（图片/视频），视频循环可控；透明度/滤镜调节，面板折叠，专栏全透+毛玻璃，编辑页排除，弹窗保真。
// @author       a_small_OIer
// @license      MIT
...
(function () {
    ...
    async function updateBg(s){
        ...
        const selected = allMedia[idx];
        await dbUpdate(selected.id , { lastUsed: now });
        try{
            const url = buf2url(selected.data , selected.mime);
